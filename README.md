...
- **Ticket types** — define any number per instance (e.g. 1-day/3-day, student, presenter), each with a price (defaults to JPY, any ISO 4217 currency — zero-decimal currencies such as JPY only accept whole-number prices), capacity, an optional *availability window* (when it can be purchased or claimed — e.g. an early-bird period — enforced server-side; a checkbox controls whether the actual dates are shown to users or just a generic "not currently available" message), and an optional cap on how many of that type a single user may hold (new types default to 1; blank means unlimited). The Manage ticket types page's Visible column is a switch, so a type can be shown/hidden instantly without opening the edit form. Deleting a ticket type also deletes its promo codes (issued tickets are unaffected). A type can be *presenter only* (checked against Conference Program — an accepted submission you speak on), or gated on membership of a course group or an enrolment method.
...
- **Badges, tickets, receipts & certificates** — organiser-edited templates (with placeholder fields drawn from the program, course, user profile and ticket cost) render to PDF, each badge carrying a unique QR code. PDFs embed a bundled Japanese-capable font so Japanese (and other non-Latin-script) template content renders correctly instead of as mojibake. Download per-attendee or as a bulk ZIP.
- **Check-in** — a camera-only web QR scanner (native `BarcodeDetector` where supported, falling back to a vendored jsQR decoder elsewhere, e.g. Safari/Firefox) records attendance; it also works inside the Moodle app. A successful scan flashes the preview green with a checkmark and plays a short (mutable) beep; the scanned name auto-clears after 3 seconds for privacy. Attendance certificates unlock once a check-in is recorded.
