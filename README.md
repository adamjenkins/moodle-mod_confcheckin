...
- **Ticket types** — define any number per instance (e.g. 1-day/3-day, student, presenter), each with a price (defaults to JPY, any ISO 4217 currency — zero-decimal currencies such as JPY only accept whole-number prices), capacity, validity window, and an optional cap on how many of that type a single user may hold (new types default to 1; blank means unlimited). Deleting a ticket type also deletes its promo codes (issued tickets are unaffected). A type can be *presenter only* (checked against Conference Program — an accepted submission you speak on), or gated on membership of a course group or an enrolment method.
