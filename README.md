...
- **Ticket types** — define any number per instance (e.g. 1-day/3-day, student, presenter), each with a price, currency, capacity and validity window. A type can be *presenter only* (checked against Conference Program — an accepted submission you speak on), or gated on membership of a course group or an enrolment method.
...
- **Check-in** — a web QR scanner (a plain text field that any USB/Bluetooth badge scanner drives, plus optional camera scanning) records attendance; it also works inside the Moodle app. Attendance certificates unlock once a check-in is recorded.
