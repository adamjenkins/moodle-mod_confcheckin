...
- **チケット種別** — インスタンスごとに任意の数（例:1日券／3日券、学生、発表者）を、価格（既定は日本円、任意の ISO 4217 通貨に変更可）・定員・有効期間、および1人が保有できるその種別のチケット数の上限（既定 1）とともに定義します。種別は*発表者限定*（Conference Program で確認 — 自分が発表する採択済み応募があること）にしたり、コースグループへの所属や登録方法でゲートしたりできます。
