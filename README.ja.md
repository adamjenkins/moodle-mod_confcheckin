...
- **チケット種別** — インスタンスごとに任意の数（例:1日券／3日券、学生、発表者）を、価格（既定は日本円、任意の ISO 4217 通貨に変更可。日本円など小数を使わない通貨は整数価格のみ受け付けます）・定員、任意の**取得可能期間**（購入・取得できる期間 — 例:早割期間。サーバー側で強制されます。実際の日付をユーザーに表示するか、単に「現在取得できません」という一般的なメッセージのみ表示するかはチェックボックスで切り替えられます）、および1人が保有できるその種別のチケット数の上限（新規種別の既定は 1、空欄は無制限）とともに定義します。「チケット種別の管理」ページの「表示」列はスイッチになっており、編集画面を開かずに種別の表示・非表示を即座に切り替えられます。チケット種別を削除すると、その種別のプロモコードも一緒に削除されます（発行済みチケットには影響しません）。種別は*発表者限定*（Conference Program で確認 — 自分が発表する採択済み応募があること）にしたり、コースグループへの所属や登録方法でゲートしたりできます。
...
- **バッジ・チケット・領収書・修了証** — 主催者が編集するテンプレート（プログラム・コース・ユーザープロフィール・チケット金額から取得したプレースホルダー付き）を PDF に描画し、各バッジには一意の QR コードが付きます。PDF には日本語対応フォントが同梱されており、日本語（および他の非ラテン文字）のテンプレート内容が文字化けせず正しく表示されます。参加者ごと、または一括 ZIP でダウンロードできます。
